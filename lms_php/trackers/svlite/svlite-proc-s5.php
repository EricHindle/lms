	...
	if($_SERVER['REQUEST_METHOD'] == 'POST')
    {
        ...
        else
        {
			if(login_check($mypdo) == true) {
				if ( isset($_POST['id'], $_POST['q14radio'], $_POST['q15radio'], $_POST['comments5'])  ) {
					...
					if ($id && $answerCheck) {
						...
						$updateSql = "UPDATE svlitechks SET na = :na, q14 = :q14, q15 = :q15, s5 = :s5, comment5 = :comment, lastcompsection = :lastSection WHERE id = :id";
						$updateQuery = $mypdo->prepare($updateSql);
						$updateQuery->execute(array(':na'=>$nonapplicable, ':q14'=>$answerArray['q14'], ':q15'=>$answerArray['q15'], ':s5'=>$sectionScore, ':comment'=>$comments, ':lastSection'=>5, ':id'=>$id));
