<?php
	...
	if(login_check($mypdo) == true) {
		if (isset($_GET['r'])) {
			...
			if ($survey_id) {
				$check_sql = "SELECT id FROM svlitechks WHERE id = :id LIMIT 1";
				$check_query = $mypdo->prepare($check_sql);
				$check_query->execute(array(':id'=>$survey_id));
				$check_count = $check_query->rowCount();
				if ($check_count>0) {
					$updateSql = "UPDATE svlitechks SET fordelete = :fordelete WHERE id = :id";
					...
					if ($updateRows>0) {
						echo "
							<script>
								...
								window.location.href='".$myPath."trackers/svlite/svlite-incomplete.php';
							</script>
						";
					} else {
						echo "
