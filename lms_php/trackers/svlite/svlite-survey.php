	...
	if(login_check($mypdo) == true) {
		if (isset($_SESSION['svid'], $_SESSION['svsec'])) {
			...
			if ($survey_id && $current_section) {
				$chk_exists_sql = "SELECT id, shop, cem FROM svlitechks WHERE id = :id LIMIT 1";
				...
				if ($chk_exists>0) {
					        	...
					        	<div class="row">
					                <section id="veform">
					                	<div class="well well-xs col-xs-12">
					                		...
					                		<form class="form-horizontal" role="form" id = "veform" name ="veform" method="post" action="svlite-proc-s'.$current_section.'.php" onSubmit="return countNA()">
					...
					$html.= '	
												<div class="panel panel-primary">
													<div class="panel-heading">'.$json['sectionNames'][$current_section-1].'</div>
												</div>
												<hr>
					';
		        	foreach ($json['questions']as $value) {
		        		if ($value['section']==$current_section) {
							$html.= '
												<h4>Q'.$value['id'].': '. $value['question']. '</h4>
			    			...
			    			if($value['info']!=""){
						    	$html.='
								    				<div class="col-xs-2">
								    					<button type="button" class="btn btn-danger pull-right" data-toggle="modal" data-target="#myModal" data-info="'.$value['info'].'">Info</button>
								    				</div>
						    	';
						    }	    
						    $html.='
					...
					$html.= '        
												<div class="form-group">
							                        <div class="col-xs-12">
							                        	...
							                        	<textarea name="comments'.$current_section.'" id="comments'.$current_section.'" data-id ="'.$current_section.'" rows="6" class="form-control" placeholder="Type your comments here"  ></textarea>
							                        </div>
							                    </div>
												<div class="form-group">
					                                <div class="col-sm-6">
		        	...
		        	$html.='
									<div class="row">
										<br>
										<div class="col-sm-12">
											<a href="svlite-main.php" name="exitBtn" id="exitBtn" onclick="return ruSure()" class="btn btn-primary btn-lg" role="button">Exit Survey</a>
											<br>
										</div>
									</div>
									<br><br>
