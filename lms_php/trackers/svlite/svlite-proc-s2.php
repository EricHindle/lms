	...
	if($_SERVER['REQUEST_METHOD'] == 'POST')
    {
        ...
        else
        {
			if(login_check($mypdo) == true) {
				if ( isset($_POST['id'], $_POST['q4radio'], $_POST['comments2'])  ) {
					...
					if ($id && $answerCheck) {
						...
						$updateSql = "UPDATE svlitechks SET na = :na, q4 = :q4, s2 = :s2, comment2 = :comment, lastcompsection = :lastSection WHERE id = :id";
						$updateQuery = $mypdo->prepare($updateSql);
						$updateQuery->execute(array(':na'=>$nonapplicable, ':q4'=>$answerArray['q4'], ':s2'=>$sectionScore, ':comment'=>$comments, ':lastSection'=>2, ':id'=>$id));
