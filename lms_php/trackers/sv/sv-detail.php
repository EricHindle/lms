...
if (login_check($mypdo) == true) {
    if (isset($_GET['s'])) {
        $id = sanitize_int($_GET['s']);
            if($id)
            {
				$sql= "SELECT * FROM svchkscomp WHERE id = :id LIMIT 1";
            ...
            if ($count > 0) {
                ...
                $sections_fetch = $sections_query->fetchAll(PDO::FETCH_ASSOC);

                foreach ($sections_fetch as $section) {
                    $html .= '
									<div class="panel panel-primary">
										...
										';
                    $answers_sql = "SELECT a.questions_id as qid, a.question_number as qno, a.answer_value as ansval, q.section as section, q.text as question from answers a join questions q on a.questions_id = q.questions_id where a.svchkscomp_id = :id and q.section = :sect order by a.question_number";
                    $answers_query = $mypdo->prepare($answers_sql);
                    $answers_query->execute(array(
                        ':id' => $id,
                        ':sect' => $section['sections_id']
                    ));
                    $answers_fetch = $answers_query->fetchAll(PDO::FETCH_ASSOC);
                    foreach ($answers_fetch as $answer) {

                        if (is_numeric($answer['ansval'])) {
                            $my_answer = $answer['ansval'] . '/5';
                        } else {
                            $my_answer = $answer['ansval'];
                        }
                        $html .= '
											<h4>' . $answer['section']. '.' . $answer['qno'] . ') ' . $answer['question'] . '</h4>
                    ...
                    $html .= '
											...
											<h4>Comments:</h4>
											<p>' . $fetch['comment' . $section['sections_id']] . '</p>
										</div>
									</div>
									<br>
								';
                ...
                $html .= '
								...
								<div class="row">
									<div class="col-xs-6">
										<a href="sv-records.php" class="btn btn-primary btn-lg" role="button">Back</a>
									</div>
									<br><br><br><br>
