		...
		if (isset($_SESSION['svid'], $_SESSION['svsec'])) {
			...
			if ($survey_id && $current_section) {
				...
				if ($chk_exists>0) {
					...
					$formKey = new formKey();
					$str = file_get_contents($myPath.'trackers/sv/questions/sv.json');
		...
		echo '				</head>
					...
					$html.= '	
												<div class="panel panel-primary">
													<div class="panel-heading">'.$json['sectionNames'][$current_section-1].'<small class = "pull-right">'.$json['sectionTimes'][$current_section].'</small></div>
												</div>
												<hr>
					';
