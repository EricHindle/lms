<?php
	...
	if(login_check($mypdo) == true) { 
		...
		switch ($role) {
			case 'BPM':
				...
				if ($_SESSION['region']=='1') {
					$place = $_SESSION['area'];
					$sql= "SELECT id, shop, score, datecomplete, veperiod FROM svchkscomp WHERE area = :place ORDER BY datecomplete DESC";
				} else {
					$place = $_SESSION['cluster'];
					$sql= "SELECT id, shop, score, datecomplete, veperiod FROM svchkscomp WHERE cluster = :place ORDER BY datecomplete DESC";
				}
				break;
			case 'AM':
				$place = $_SESSION['area'];
			...
			case 'RM':
				...
				$sql= "SELECT id, shop, score, datecomplete, veperiod FROM svchkscomp WHERE region = :place ORDER BY datecomplete DESC";
				break;
			case 'DD':
				$place = $_SESSION['division'];
				$sql= "SELECT id, shop, score, datecomplete, veperiod FROM svchkscomp WHERE division = :place ORDER BY datecomplete DESC";
				break;
			case 'Admin':
				$place = $_SESSION['division'];
				$sql= "SELECT id, shop, score, datecomplete, veperiod FROM svchkscomp WHERE division = :place ORDER BY datecomplete DESC";
		...
		if($count>0){
			...
			echo '
						<h1>Smart Visit<small class="pull-right">Reports</small></h1>
						<div class="row">
							<div class="col-xs-12">
								<table class="table table-striped table-bordered" id="keywords" >
									...
									foreach ($fetch as $myResult)
									{
										...
										echo '<td><a href="sv-detail.php?s='.$myResult['id'].'">SV-'.$myResult['id'].'</a></td>';
