                ...
                if ($teamid && $teamname) {
                    ...
                    if ($teamcount > 0) {
                        ...
                        foreach ($abbrlist as $abbr) {
                            $dupct += check_dup_abbr($teamid, $abbr);
                        }
                        if ($dupct == 0){
                            $abbrct = remove_abbr_for_team($teamid);
                            foreach ($abbrlist as $abbr) {
                                $abbrct += insert_team_abbr($teamid, $abbr);
                            }
                            if ($abbrct > 0) {
                                 $alerttext .= "Abbr added to team.";
                            }
                            ...
                            if ($upcount > 0 || $leagueadded > 0 || $abbrct > 0) {
                                $html .= "<script>
    											alert('".$alerttext."');
                            ...
                            $html .= "<script>
    										alert('Duplicate abbreviation(s). Not updated.');
    										window.location.href='team-main.php';
    							     </script>";
                        }
                    } else {
